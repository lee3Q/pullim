...
| 지표 | 원래 | CC 후 | 업계 벤치마크 |
|------|------|-------|-------------|
| 세션 완주율 | 60% | **40%** | 멘탈헬스 앱 완주율 데이터 부족 |
| 7일 내 재방문율 | 25% | **15~20%** | Day 30 리텐션 중앙값 3.3% (!!) |
| "매우 실망" 응답 | 40% | **30%** | Sean Ellis 기준 40% = PMF 신호 |

**충격적 데이터**: 멘탈헬스 앱 Day 30 리텐션 중앙값이 **3.3%** (일반 앱 15%+). 풀림이 15-20% 재방문을 달성하면 이미 업계 상위권.
(출처: [PMC Mental Health Apps Engagement](https://pmc.ncbi.nlm.nih.gov/articles/PMC8844980/))

**CC 보강 (Sonnet): Auth를 Phase 1으로 당겨야 함.**
- localStorage로 10명은 임시 운영 가능하지만, PMF 재방문 지표를 신뢰하려면 Auth 필요
- Phase 2(50명)에서 데이터 신뢰 문제 발생하므로, Phase 1 내에 Supabase Auth 구현 권장
