...
- 매운맛 spicyLabel 전면 교체 — "짜치다" 피드백. DC 말투→패턴 직설 통찰로. 도입 방식 고민 중
- 나만의 느낌 카드 — 파악 후 경향성 표시. API 불필요(로컬 axis). 기록 저장과 함께
- sw.js 캐시 무제한 저장 — GET 전부 캐시 → 용량 폭발. URL 필터링 or maxEntries 제한
- **5번째 테마: 아포칼립스** — 극한 상황 속 감성. "세상에 남은 너와 나", 살아남으며 느끼는 것. 세계관 설계 후 이미지 45장 생성 예정
...
- 상태: 진행 중
- 최종 갱신: 2026-04-01
- 2026-04-01: **종말 파악 이미지 매핑 15개 수정** — 이미지 파일은 정상이었으나 story-scenes.ts 번호가 2~3칸씩 밀려있었음. 전수 대조 후 재매핑. **대화형 뷰 배경 이미지 숨김** — 카드형에서만 표시. Vercel 수동 배포 완료(d696580). **Vercel 연쇄 취소 원인 파악**: 빠른 연속 push 시 이전 빌드 자동 취소 → 마지막 빌드도 취소되는 문제. 해결: 작업 완료 후 `vercel deploy --prod` 수동 실행.
- 2026-03-31: **Vercel 배포 정상화** (30커밋 누락→수동배포+GitHub 연동). UI 긴급 수정 8건: BGM 홈+세션 추가, response-parser HTML태그 제거, 파악 진입 복원(테마별 파악/바로대화 분기), 카드형 뷰(이전/다음+대화형 토글), 사용자 선택 밝기, footer 겹침 제거.
