...
const theme = getTheme("종말");

// 새 에셋 (themes/index.ts 수정 금지이므로 여기서 오버라이드)
const AVATAR_SRC = "/assets/apocalypse-avatar.png";
const STAGE_BACKGROUNDS: Partial<Record<string, string>> = {
  ENTER: "/assets/apocalypse-enter-bg.png",
  LISTEN: "/assets/apocalypse-listen-bg.png",
  RESEARCH: "/assets/apocalypse-research-bg.png",
  VERIFY: "/assets/apocalypse-listen-bg.png",
  DISCUSS_1: "/assets/apocalypse-listen-bg.png",
  CRYSTAL_SELECT: "/assets/apocalypse-crystal-bg.png",
  CRYSTAL_ANALYZE: "/assets/apocalypse-crystal-bg.png",
  DISCUSS_2: "/assets/apocalypse-crystal-bg.png",
  DEBATE: "/assets/apocalypse-crystal-bg.png",
  DISCUSS_3: "/assets/apocalypse-crystal-bg.png",
  JUDGE: "/assets/apocalypse-crystal-bg.png",
  CONCLUDE: "/assets/apocalypse-conclude-bg.png",
  COMPLETE: "/assets/apocalypse-conclude-bg.png",
};

const ERROR_MSG =
  "신호가 잠시 끊겼어요... 다시 한번 말씀해주시겠어요?";
...
export default function ApocalypseSessionPage() {
  ...
  const callAnalyzeAPI = async (crystals: CrystalName[]) => {
    ...
    try {
      ...
      const crystalOptions = analyses.map((a: CrystalAnalysis) => {
        ...
        return `${c?.icon || "⭐"} ${a.crystal}의 파편`;
      });
      setOptions(crystalOptions);
      setStage("DISCUSS_2");
      advanceScene("파편이 이야기를 보여주네요.\n어떤 이야기가 가장 와닿으셨나요?");
  ...
  const callDebateAPI = async (selectedCrystal: CrystalName) => {
    ...
    try {
      const response = await fetch("/api/ultimate/debate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ analyses: crystalAnalyses, disagreements, selectedCrystal, listenSummary, crystalModelMap, theme: "종말" }),
      });
      if (!response.ok) { setScene(ERROR_MSG); setIsStreaming(false); return; }
      const data = await response.json();
      ...
      if (data.skipped) {
        ...
        advanceScene(data.synthesis || "파편이 정리해줬어요.\n\n어떻게 느껴지세요?");
      ...
      advanceScene((data.synthesis || "파편의 이야기가 끝났어요.") + "\n\n어떻게 느껴지세요?");
  ...
  const callConcludeAPI = async () => {
    ...
    try {
      const response = await fetch("/api/ultimate/conclude", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ concern, listenSummary, analyses: crystalAnalyses, disagreements, debateRounds, debateSynthesis, userName, theme: "종말" }),
      });
      if (!response.ok) { setScene(ERROR_MSG); setIsStreaming(false); return; }
      const data = await response.json();
      if (data.crisis) { setCrisis({ message: data.message, hotline: data.hotlines?.[0]?.number || "109", riskState: data.policy?.riskState }); setIsStreaming(false); return; }
  ...
  const handleCrystalConfirm = (crystals: CrystalName[]) => {
    ...
    setScene("파편이 빛나기 시작해요. 잠깐 기다려주세요.");
    ...
    <div className="min-h-[100dvh] flex flex-col relative overflow-x-hidden overflow-y-auto w-full max-w-[100vw]">
      ...
      {isGameMode && (
        <>
          ...
          <img
            src={STAGE_BACKGROUNDS[stage] || "/assets/apocalypse-enter-bg.png"}
            alt=""
