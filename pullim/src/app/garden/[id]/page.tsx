...
const AVATAR_SRC = "/assets/garden-rabbit.png";
const STAGE_BACKGROUNDS: Partial<Record<string, string>> = {
  ENTER: "/assets/garden-enter-bg.png",
  LISTEN: "/assets/garden-listen-bg.png",
  RESEARCH: "/assets/garden-research-bg.png",
  VERIFY: "/assets/garden-research-bg.png",
  DISCUSS_1: "/assets/garden-research-bg.png",
  CRYSTAL_SELECT: "/assets/garden-crystal-bg.png",
  CRYSTAL_ANALYZE: "/assets/garden-crystal-bg.png",
  DISCUSS_2: "/assets/garden-crystal-bg.png",
  DEBATE: "/assets/garden-crystal-bg.png",
  DISCUSS_3: "/assets/garden-crystal-bg.png",
  JUDGE: "/assets/garden-crystal-bg.png",
  CONCLUDE: "/assets/garden-conclude-bg.png",
  COMPLETE: "/assets/garden-conclude-bg.png",
};

const ERROR_MSG =
  "꽃봉오리가 잠시 잠들었어요... 다시 한번 이야기해줄 수 있어요?";
...
export default function GardenSessionPage() {
  ...
  const callAnalyzeAPI = async (crystals: CrystalName[]) => {
    ...
    try {
      ...
      setStage("DISCUSS_2");
      advanceScene("꽃봉오리들이 이야기를 들려줬어요.\n어떤 이야기가 가장 마음에 와닿았어요?");
  ...
  const callDebateAPI = async (selectedCrystal: CrystalName) => {
    ...
    try {
      const response = await fetch("/api/ultimate/debate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ analyses: crystalAnalyses, disagreements, selectedCrystal, listenSummary, crystalModelMap, theme: "달빛정원" }),
      });
      if (!response.ok) { setScene(ERROR_MSG); setIsStreaming(false); return; }
      const data = await response.json();
      if (data.crisis || data.debateBlocked) { advanceScene("이 마음에는 다른 종류의 돌봄이 필요해 보여요."); setIsStreaming(false); return; }
      if (data.skipped) {
        setDebateRounds([]); setDebateSynthesis(data.synthesis || ""); setStage("DISCUSS_3"); setIsStreaming(false);
        advanceScene(data.synthesis || "꽃봉오리가 정리해줬어요.\n\n어떻게 느껴져요?");
      ...
      advanceScene((data.synthesis || "꽃봉오리들의 이야기가 끝났어요.") + "\n\n어떻게 느껴져요?");
  ...
  const callConcludeAPI = async () => {
    ...
    try {
      const response = await fetch("/api/ultimate/conclude", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ concern, listenSummary, analyses: crystalAnalyses, disagreements, debateRounds, debateSynthesis, userName, theme: "달빛정원" }),
      });
      if (!response.ok) { setScene(ERROR_MSG); setIsStreaming(false); return; }
      const data = await response.json();
  ...
  const handleCrystalConfirm = (crystals: CrystalName[]) => {
    ...
    setScene("꽃봉오리가 피어나고 있어요. 잠깐만 기다려줘요.");
    ...
    <div className="min-h-[100dvh] flex flex-col relative overflow-x-hidden overflow-y-auto w-full max-w-[100vw]">
      ...
      {isGameMode && (
        <>
          ...
          <img
            src={STAGE_BACKGROUNDS[stage] || "/assets/garden-enter-bg.png"}
            alt=""
