...
const theme = getTheme("천문대");

// 새 에셋 (themes/index.ts 수정 금지이므로 여기서 오버라이드)
const AVATAR_SRC = "/assets/stargazer-avatar.png";
const STAGE_BACKGROUNDS: Partial<Record<string, string>> = {
  ENTER: "/assets/stargazer-enter-bg.png",
  LISTEN: "/assets/stargazer-listen-bg.png",
  RESEARCH: "/assets/stargazer-research-bg.png",
  VERIFY: "/assets/stargazer-listen-bg.png",
  DISCUSS_1: "/assets/stargazer-listen-bg.png",
  CRYSTAL_SELECT: "/assets/stargazer-crystal-bg.png",
  CRYSTAL_ANALYZE: "/assets/stargazer-crystal-bg.png",
  DISCUSS_2: "/assets/stargazer-crystal-bg.png",
  DEBATE: "/assets/stargazer-crystal-bg.png",
  DISCUSS_3: "/assets/stargazer-crystal-bg.png",
  JUDGE: "/assets/stargazer-crystal-bg.png",
  CONCLUDE: "/assets/stargazer-conclude-bg.png",
  COMPLETE: "/assets/stargazer-conclude-bg.png",
};

const ERROR_MSG =
  "별빛이 잠시 흐려졌어요... 다시 한번 말씀해주시겠어요?";
const FREE_INPUT_LABEL = "직접 말하기";

const DEFAULT_CHOICES: Partial<Record<string, string[]>> = {
  ENTER: ["방향을 잃은 것 같아요", "가능성을 찾고 싶어요", FREE_INPUT_LABEL],
  LISTEN_FALLBACK: ["더 이야기할게요", "여기까지예요", FREE_INPUT_LABEL],
  DISCUSS_1: ["도움이 되는 정보네요", "제 상황과는 좀 달라요", FREE_INPUT_LABEL],
  DISCUSS_3: ["정리가 됐어요", "다시 생각해볼게요"],
};
...
export default function StargazerSessionPage() {
  ...
  useEffect(() => {
    if (userProfile.profile && !userProfile.isNewUser) {
      ...
      if (userProfile.profile.userName) {
        const name = userProfile.profile.userName;
        window.setTimeout(() => setUserName(name), 0);
      }
    }
  ...
  if (isLadder) {
    return <LadderSessionPage theme="천문대" bgImage="/assets/stargazer-listen-bg.png" />;
  }
  ...
  const callResearchAPI = async (summary: string, currentConcern?: string) => {
    ...
    try {
      ...
      setIsStreaming(false);
      const discuss1Text = "별자리를 검증해봤어요.\n어떻게 보이세요?";
  ...
  const callAnalyzeAPI = async (crystals: CrystalName[]) => {
    ...
    try {
      ...
      const crystalOptions = analyses.map((a: CrystalAnalysis) => {
        ...
        return `${c?.icon || "⭐"} ${a.crystal}의 별의 조각`;
      });
      setOptions(crystalOptions);
      setStage("DISCUSS_2");
      advanceScene("별의 조각이 이야기를 보여주네요.\n어떤 이야기가 가장 와닿으셨나요?");
  ...
  const callDebateAPI = async (selectedCrystal: CrystalName) => {
    ...
    try {
      const response = await fetch("/api/ultimate/debate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ analyses: crystalAnalyses, disagreements, selectedCrystal, listenSummary, crystalModelMap, theme: "천문대" }),
      });
      if (!response.ok) { setScene(ERROR_MSG); setIsStreaming(false); return; }
      const data = await response.json();
      if (data.crisis || data.debateBlocked) { advanceScene("이 방향에는 다른 종류의 안내가 필요할 것 같아요."); setIsStreaming(false); return; }
      if (data.skipped) {
        setDebateRounds([]); setDebateSynthesis(data.synthesis || ""); setStage("DISCUSS_3"); setIsStreaming(false);
        advanceScene(data.synthesis || "별의 조각이 정리해줬어요.\n\n어떻게 느껴지세요?");
      ...
      advanceScene((data.synthesis || "별의 조각의 이야기가 끝났어요.") + "\n\n어떻게 느껴지세요?");
  ...
  const callConcludeAPI = async () => {
    ...
    try {
      const response = await fetch("/api/ultimate/conclude", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ concern, listenSummary, analyses: crystalAnalyses, disagreements, debateRounds, debateSynthesis, userName, theme: "천문대" }),
      });
      if (!response.ok) { setScene(ERROR_MSG); setIsStreaming(false); return; }
      const data = await response.json();
      if (data.crisis) { setCrisis({ message: data.message, hotline: data.hotlines?.[0]?.number || "109", riskState: data.policy?.riskState }); setIsStreaming(false); return; }
  ...
  const handleCrystalConfirm = (crystals: CrystalName[]) => {
    ...
    setScene("별의 조각이 빛나기 시작해요. 잠깐 기다려주세요.");
    ...
    <div className="min-h-[100dvh] flex flex-col relative overflow-x-hidden overflow-y-auto w-full max-w-[100vw]">
      ...
      {isGameMode && (
        <>
          ...
          <img
            src={STAGE_BACKGROUNDS[stage] || "/assets/stargazer-enter-bg.png"}
            alt=""
